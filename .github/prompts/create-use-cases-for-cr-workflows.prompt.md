# Create Use Cases for Cyber Resilience Workflows

## Prompt

Create use cases for the workflows defined in the Del Cyber Resilience management workflows.

## Context

The growing sophistication of ransomware and advanced cyber threats requires a defense-in-depth approach to protecting enterprise storage
This Proof of Concept (PoC) aims to demonstrate an end-to-end cyber resilience platform centered on Dell PowerStore as the protected target, leveraging multiple detection and protection layers.

## Goals
- Demonstrate multi-layered detection (inline, nearline, backup, SaaS) with consistent alerting and event correlation.
- Prove rapid response capabilities - Realtime snapshot creation and user lockout.
- Validate recovery readiness - restore from snapshots and backups while verifying data integrity (2nd priority).
- Showcase integrations with Wazuh SIEM for visibility, correlation, and SOAR automation.
- Use realistic workloads and attack simulations that clearly illustrate the business impact and recovery process.

## Source Material

### Workflows to Convert

The following workflows from `.github/cr-requirements/use-cases/cr-workflows.md` need use cases created:

1. **Data Generation and Transition Workflows:**

   - Generate Normal Workload, which is used to train detection algorithms to adapt to the customer's normality and offer better performance.
   - Simulate/Emulate Ransomware Attack

2. **Attack Detection Workflows:**
   - Detect Malicious iSCSI Read/Write Commands Patterns
   - Fetch Metrics Data from PowerStore Server
   - Detect Suspicious Ransomware Activity in PPDM
   - Detect Ransomware in Nearline (Decision Point)


3. **Response And Recover Workflows:**

   - Pack Incident Response Data in Case a Ransomware Activity is Detected
   - Initial Investigation of a Security Incident in the SIEM
   - Execute a Playbook in XDR/SOAR
   - Execution of Response Actions in Nearline
...
- Store files in `cr-requirements/use-cases/` directory
